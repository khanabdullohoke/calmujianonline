...
function renderUjianList() {
  ...
  wrap.innerHTML = ujianCache.map((u) => {
    ...
    const statusBadge = u.status === "aktif" ? '<span class="badge badge-green">Aktif</span>' : '<span class="badge badge-gray">Draft</span>';
    return `
    <div class="card mb-16">
      <div class="flex-between">
        <div>
          <div style="font-weight:600;font-size:15px;">${esc(u.judul)} ${statusBadge}</div>
          <div class="hint">${u.soal.length} soal · ${u.durasiMenit} menit · Kelas: ${esc(namaKelas)}</div>
        </div>
      </div>
...
async function loadHasil() {
  ...
  tbody.innerHTML = "";
  ringkasan.innerHTML = "";
  if (!ujianId) { document.getElementById("hasilEmpty").style.display = "block"; return; }
  ...
  if (rows.length > 0) {
    ...
    ringkasan.innerHTML = `
      <div class="card center"><div class="hint">Rata-rata</div><div style="font-size:26px;font-weight:700;">${rata}</div></div>
      <div class="card center"><div class="hint">Nilai Tertinggi</div><div style="font-size:26px;font-weight:700;color:#1F7A46;">${tertinggi}</div></div>
      <div class="card center"><div class="hint">Nilai Terendah</div><div style="font-size:26px;font-weight:700;color:#E15554;">${terendah}</div></div>
    `;
  }
}
